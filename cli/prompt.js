  ...
  function render() {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item.type === 'separator') {
        ...
        out += `${prefix} ${check} ${active ? BOLD : ''}${item.label}${RESET}${desc}\n`;
      }
    }
    return out;
  }
  ...
  function render() {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item.type === 'separator') {
        ...
        const desc = item.description ? `  ${DIM}${item.description}${RESET}` : '';
        out += `${prefix} ${dot} ${active ? BOLD : ''}${item.label}${RESET}${desc}\n`;
      }
    }
    return out;
  }
...
async function fallbackMultiSelect(title, items, defaultSelected) {
  ...
  for (const item of items) {
    ...
    const ans = await ask(`  Include ${item.label}${desc}? (${def}) `);
...
async function fallbackSingleSelect(title, items) {
  ...
  console.log(`\n${title}`);
  active.forEach((item, i) => console.log(`  ${i + 1}. ${item.label}${item.description ? ` — ${item.description}` : ''}`));
